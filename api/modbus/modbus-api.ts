...
export class ModbusAPI {
  ...
  readAddress = async (register: ModbusRegister): Promise<any> => {
    const input = await this.client.readInputRegisters(
      register.address,
      register.length,
    );
    ...
    const result = this.deviceDefinition.inputRegisterResultConversion(
      this.log,
      input,
      register,
    );
  ...
  readRegisters = async () => {
    ...
    for (const register of this.deviceDefinition.holdingRegisters) {
      try {
        const input = await this.client.readHoldingRegisters(
          register.address,
          register.length,
        );
        const result = this.deviceDefinition.holdingRegisterResultConversion(
          this.log,
          input,
          register,
        );
