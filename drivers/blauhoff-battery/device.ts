...
class BlauHoffBattery extends Device {
    ...
    refreshUserTokenFromSettings = async (): Promise<boolean> => {
        const { accessId, accessSecret, baseUrl } = this.getSettings();
    ...
    async onInit() {
        ...
        const { userToken, baseUrl } = this.getSettings();
    ...
    private runMode = async (
        device: BlauHoffDevice,
        args: any,
        method: (device: BlauHoffDevice, args: any) => Promise<QueryResponse<boolean>>,
        comingFromError: boolean,
    ): Promise<boolean> => {
        if (!this.device) {
            return false;
        }
